...
- Never pass `width`/`height` to `Image` (lint-enforced) — use `fill` in a sized wrapper via `IllustrationImage` (`src/components/illustration-image.tsx`), especially for SVGs.
