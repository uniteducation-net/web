...
export function generateStaticParams() {
  return getSlugs("terms").map((slug) => ({ slug }));
}
...
const load = async (params: Promise<{ lang: string; slug: string }>) => {
  ...
  const entry = await getContent("terms", slug, lang);
  if (!entry) notFound();
    ...
    <LegalArticle
      ...
      breadcrumb={
        <BreadcrumbNav
          ...
          homeHref={`/${lang}`}
          items={[
            { label: dict.legalOverview.heading, href: `/${lang}/terms` },
          ]}
          options={dict.legalOverview.items.map((item) => ({
            label: item.title,
            href: `/${lang}/terms/${item.id}`,
          }))}
          selectedHref={`/${lang}/terms/${slug}`}
