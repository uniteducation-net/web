...
import { getAllContent, getAsset } from "@/lib/content";
...
export default async function TeamPage({
  ...
  const assets = await Promise.all(
    members.map(async (member) => ({
      image: await getAsset("team", member.slug, member.frontmatter.image),
      hoverImage: await getAsset(
        "team",
        member.slug,
        member.frontmatter.hoverImage,
      ),
    })),
  );
      ...
      <div className="mt-10 grid gap-x-8 gap-y-12 sm:grid-cols-2 lg:grid-cols-3">
        {members.map((entry, index) => (
          <TeamCard
            ...
            hoverImage={assets[index].hoverImage}
          />
        ))}
      </div>
    </section>
